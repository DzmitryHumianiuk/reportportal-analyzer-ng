...
Scored **200** probe items from launches {'webshop-ui': 164, 'payments-services': 165, 'frontend-apps': 166} against `expected.json`. Debatable (expectation/timing artifacts, excluded from fault counts): **57**.

## Summary

| scope | n | action acc | label acc (decided) | confidently-wrong | abstain recall | method match |
|---|---:|---:|---:|---:|---:|---:|
| **overall** | 143 | 0.734 | 0.745 | 12 | 0.763 | 0.288 |
| webshop-ui | 90 | 0.733 | 0.853 | 6 | 0.385 | 0.287 |
| payments-services | 25 | 0.68 | 0.571 | 6 | 1.0 | 0.5 |
| frontend-apps | 28 | 0.786 | 0.111 | 0 | 0.938 | 0.067 |
...
| S21 | 6 | PARTIAL | 1.0 | 0.667 | 0 | mixed |
...
| S33 | 15 | PARTIAL | 0.667 | 0.6 | 0 | mixed |
...
| ADV-6 | PARTIAL | 6 items, action_acc=1.0, conf_wrong=0 |
...
1. **JAVA-SEL-12** (S37/ADV-4) item 1977 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
2. **JAVA-SEL-12** (S37/ADV-4) item 1979 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
3. **JAVA-SEL-12** (S37/ADV-4) item 1981 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
4. **JAVA-SEL-12** (S37/ADV-4) item 1983 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
5. **JAVA-SEL-12** (S37/ADV-4) item 1985 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
6. **JAVA-SEL-12** (S37/ADV-4) item 1987 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
7. **NET-XUN-03** (S18/ADV-3) item 2058 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
8. **NET-XUN-03** (S18/ADV-3) item 2059 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
9. **NET-XUN-03** (S18/ADV-3) item 2062 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
10. **NET-XUN-08** (S41) item 2075 `Hawkins.Payments.MoneyMovement.Tests.InstantPayoutsE2E`  
...
- 57× — spec expected suggest; analyzer auto-labeled the CORRECT label (post-replay history richer than the spec's suggest assumption)
