...
Scored **200** probe items from launches {'webshop-ui': 170, 'payments-services': 171, 'frontend-apps': 172} against `expected.json`. Debatable (expectation/timing artifacts, excluded from fault counts): **59**.

## Summary

| scope | n | action acc | label acc (decided) | confidently-wrong | abstain recall | method match |
|---|---:|---:|---:|---:|---:|---:|
| **overall** | 141 | 0.645 | 0.852 | 12 | 0.816 | 0.303 |
| webshop-ui | 88 | 0.648 | 0.906 | 6 | 0.462 | 0.308 |
| payments-services | 25 | 0.68 | 0.571 | 6 | 1.0 | 0.5 |
| frontend-apps | 28 | 0.607 | 1.0 | 0 | 1.0 | 0.067 |
...
| S18 | 9 | FAIL | 0.222 | 0.0 | 3 | hard fail |
...
| S33 | 15 | PARTIAL | 0.733 | 1.0 | 0 | mixed |
...
| ADV-3 | FAIL | 9 items, action_acc=0.222, conf_wrong=3 |
...
1. **JAVA-SEL-12** (S37/ADV-4) item 2377 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
2. **JAVA-SEL-12** (S37/ADV-4) item 2379 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
3. **JAVA-SEL-12** (S37/ADV-4) item 2381 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
4. **JAVA-SEL-12** (S37/ADV-4) item 2383 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
5. **JAVA-SEL-12** (S37/ADV-4) item 2385 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
6. **JAVA-SEL-12** (S37/ADV-4) item 2387 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
7. **NET-XUN-03** (S18/ADV-3) item 2465 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
8. **NET-XUN-03** (S18/ADV-3) item 2466 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
9. **NET-XUN-03** (S18/ADV-3) item 2467 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
10. **NET-XUN-08** (S41) item 2475 `Hawkins.Payments.MoneyMovement.Tests.InstantPayoutsE2E`  
