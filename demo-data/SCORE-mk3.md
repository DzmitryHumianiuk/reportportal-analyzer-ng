...
Scored **200** probe items from launches {'webshop-ui': 167, 'payments-services': 168, 'frontend-apps': 169} against `expected.json`. Debatable (expectation/timing artifacts, excluded from fault counts): **61**.

## Summary

| scope | n | action acc | label acc (decided) | confidently-wrong | abstain recall | method match |
|---|---:|---:|---:|---:|---:|---:|
| **overall** | 139 | 0.719 | 0.717 | 12 | 0.684 | 0.327 |
| webshop-ui | 88 | 0.727 | 0.849 | 6 | 0.385 | 0.308 |
| payments-services | 23 | 0.652 | 0.5 | 6 | 0.778 | 0.714 |
| frontend-apps | 28 | 0.75 | 0.1 | 0 | 0.875 | 0.067 |
...
1. **JAVA-SEL-12** (S37/ADV-4) item 2177 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
2. **JAVA-SEL-12** (S37/ADV-4) item 2179 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
3. **JAVA-SEL-12** (S37/ADV-4) item 2181 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
4. **JAVA-SEL-12** (S37/ADV-4) item 2183 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
5. **JAVA-SEL-12** (S37/ADV-4) item 2185 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
6. **JAVA-SEL-12** (S37/ADV-4) item 2187 `Account. Session. Reject expired session cookie`  
   expected **suggest/ab/gbm**, got **auto/pb/hash conf=0.95** — HARD FAIL: auto-labeled pb but truth is ab -- confident wrong label
7. **NET-XUN-03** (S18/ADV-3) item 2265 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/gbm conf=0.83** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
8. **NET-XUN-03** (S18/ADV-3) item 2266 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/gbm conf=0.83** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
9. **NET-XUN-03** (S18/ADV-3) item 2267 `Hawkins.Payments.Transfers.Tests.TransferApiTests.Post`  
   expected **suggest/si/gbm**, got **auto/pb/gbm conf=0.83** — HARD FAIL: auto-labeled pb but truth is si -- confident wrong label
10. **NET-XUN-08** (S41) item 2275 `Hawkins.Payments.MoneyMovement.Tests.InstantPayoutsE2E`  
   expected **suggest/nd/gbm**, got **auto/pb/gbm conf=0.83** — HARD FAIL: auto-labeled pb but truth is nd -- confident wrong label

## EXPECTED-DEBATABLE (not counted as analyzer failures)

- 61× — spec expected suggest; analyzer auto-labeled the CORRECT label (post-replay history richer than the spec's suggest assumption)
