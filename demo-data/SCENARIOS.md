...
### 2.8 Make Decision showcase (one-launch modal walkthrough)

| ID | Scenario | Proj / FW | Label | Mechanisms (source) | Adversarial notes | Volume | Expected behavior |
|---|---|---|---|---|---|---|---|
| **S46** | **Make Decision Showcase launch** (WSU, demo day 21:00): ONE launch whose 18 probe items walk every Make Decision / Bench branch — (1) `auto-hash` exact-hash inherit (JAVA-SEL-03 v0); (2) `auto-kb` confirmed-mode short-circuit (JAVA-SEL-23); (3) `suggest-confirm` fp-drift suggest band (JAVA-SEL-24); (4) `suggest-disagree` split pb/ab history → conflicting suggest rows (JAVA-SEL-27 v0); (5) `declined-dock` same conflict family, calibrated p* < τ_suggest → below_suggest row (JAVA-SEL-27 v2); (6) `ai-guess-only` never-seen `HoloLedgerSkewError` → classical abstain, cold-start rubric only (JAVA-SEL-32); (7) `silent-no-signal` failed item with INFO/DEBUG logs only → empty signature (JAVA-SEL-33); (8) `group-of-3` three S05 param variants, one error_hash → exact-group cue (JAVA-SEL-03 v1–v3); (9) `burst-si` 8 fresh-fingerprint relay failures, 8/17 indexed share > 0.40 → si burst fan-out (JAVA-SEL-34) | WSU | mixed | every Bench band (`auto`/`suggest`/`below_suggest`/rubric), agreement states S1–S4, launch_group solo/shared/burst, empty-signature silence — all inside one launch | dock case needs `ANALYZER_SUGGEST_BELOW_ENABLED=true` and a band-aware UI (service-ui ≥ ng4); JAVA-SEL-32/33/34 are placed ONLY here (POLICY target=0) so their fingerprints stay novel — re-running the showcase after history mutates weakens novelty | 1 launch × 18 P | walk the launch top-to-bottom in the Make Decision modal and see, in order: agree+Past decision, KB auto, suggest-confirm, disagree, declined dock, AI-guess-only, silent modal, 3-member exact group cue, 8-member burst group auto-`si`; items carry `scenario:BENCH-<case>` attributes for scoring |
| **S46b** | **Make Decision Showcase 2 launch** (WSU, demo day 22:00): 4 probe items that re-land the four branches the original showcase lost to the auto-hash collapse (its cases 3, 4, 5 and 6 reused JAVA-SEL-24/27 whose other items were human labeled, so stage-A exact hash won; JAVA-SEL-32 pulled 0.92 neighbors through shared TestNG boilerplate frames). New S46-only families JAVA-SEL-35..38 (POLICY target=0) follow a hash-drift rule: the showcase variant's first ERROR line differs from every history variant in words that survive masking, frames shared only where fuzzy similarity is wanted. History arrives in three dedicated phase-2 launches: `Bench History Confirm` (Jul 8, SEL-35 v1..v3 twice, unanimous pb), `Bench History Split` (Jul 8, SEL-36 v1..v4 pb + v5..v8 ab), `Bench History Weak` (Jul 9, SEL-37 v1..v2 pb heavy drift + v3..v4 gt=ti left unlabeled). Cases: (1) `suggest-confirm` JAVA-SEL-35 v0; (2) `suggest-disagree` JAVA-SEL-36 v0; (3) `declined-dock` JAVA-SEL-37 v0; (4) `ai-guess-only` JAVA-SEL-38 (alien exception, unseen `io.hawkinslab.*` frames, no TestNG boilerplate) | WSU | mixed | suggest band with unanimous history, suggest band with conflicting pb/ab rows, `below_suggest` decline dock, cold-start rubric solo; all reachable on a live stand via NEW uploads only (existing launches never edited) | hash drift must survive masking (message words or exception subtype, never only numbers/ids/paths); SEL-37 needs `ANALYZER_SUGGEST_BELOW_ENABLED=true`; upload order matters: history launches, then replay, then analyze-history, then replay again, then the showcase (exact command sequence in README 3b) | 3 launches x 18 H (6+8+4, plus filler) + 1 launch x 4 P | confirm item: method != hash, band=suggest, unanimous pb, conf [0.45, 0.75); disagree item: band=suggest, two defect groups in rows; declined item: rows carry ek=decline, conf [0.30, 0.45), decision band=below_suggest; ai-guess item: reply holds only the coldstart_rubric row; no new item ever inherits via exact hash |
...
| Wed Jul 8 | 3 nightlies + 2 smoke (10 f) + **S46b `Bench History Confirm` (6 H) and `Bench History Split` (8 H)** | nightly + 2 contract (5 f) | PW nightly + Cypress weekly (5+3 f) | S23 spam items (A/B + passing decoys); S20 multilingual batch; S16 timeout A/B populations grow |
| Thu Jul 9 | 3 nightlies + 2 smoke (8 f) + **S46b `Bench History Weak` (4 H, two stay unlabeled)** | nightly + 1 contract (3 f) | PW nightly (4 f) | S10 fx-rates fixed. Robin quarantines 3 flaky specs (few `ab` events — FEA stays < 20 total). S06 template-widening stream continues |
