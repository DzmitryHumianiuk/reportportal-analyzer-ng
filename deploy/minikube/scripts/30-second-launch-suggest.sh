#!/usr/bin/env bash
# G5 step 3, adapted for minikube (see 10-report-launch.sh header).
# Reports a SECOND launch with the same failure signature, then asks the
# analyzer for suggested items ("Make Decision") on the new item.
set -uo pipefail
...
L2=$(curl -s -X POST "$API_BASE/api/v2/$PROJECT/launch" -H "$AUTH" -H 'Content-Type: application/json' -d "{
  \"name\":\"analyzer-ng-g5-demo\",\"startTime\":$N,\"mode\":\"DEFAULT\"}" | python3 -c 'import sys,json;print(json.load(sys.stdin)["id"])')
I2=$(curl -s -X POST "$API_BASE/api/v2/$PROJECT/item" -H "$AUTH" -H 'Content-Type: application/json' -d "{
...
curl -s -X PUT "$API_BASE/api/v2/$PROJECT/item/$I2" -H "$AUTH" -H 'Content-Type: application/json' -d "{
  \"launchUuid\":\"$L2\",\"endTime\":$((N+300)),\"status\":\"FAILED\",\"issue\":{\"issueType\":\"ti001\"}}" >/dev/null
curl -s -X PUT "$API_BASE/api/v2/$PROJECT/launch/$L2/finish" -H "$AUTH" -H 'Content-Type: application/json' -d "{\"endTime\":$((N+400))}" >/dev/null
echo "launch2 uuid=$L2 item2 uuid=$I2"
sleep 8
NID=$(curl -s "$API_BASE/api/v1/$PROJECT/item/uuid/$I2" -H "$AUTH" | python3 -c 'import sys,json;print(json.load(sys.stdin)["id"])')
echo "item2 numeric id=$NID"

hr "STEP 11  Make Decision -> suggested items for the new item $NID"
curl -s "$API_BASE/api/v1/$PROJECT/item/suggest/$NID" -H "$AUTH" | jqp
echo "$NID" > "$HERE/.nid"
