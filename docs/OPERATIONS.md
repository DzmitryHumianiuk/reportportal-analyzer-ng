...
| `modelInfo` shows `emb=none`, `emb_model_ver:null` | The ONNX embedder is not contributing to scoring (retrieval falls back to lexical/signature features — still correct, but weaker). Check `ANALYZER_EMB_MODEL_PATH` resolves inside the container (`/opt/analyzer/models/e5-small-int8`) and inspect logs for embedder init. Known gap: `main.py` does not wire the loaded model tag into `/health` (reports null even when loaded). |
