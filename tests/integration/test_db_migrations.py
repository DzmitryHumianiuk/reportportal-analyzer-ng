...
def test_clean_bootstrap_creates_db_schema_and_migration(fresh_db_dsn: str) -> None:
    ...
    assert applied == [1, 2, 3, 5]

    with psycopg.connect(fresh_db_dsn) as conn:
        conn.execute("SET search_path = analyzer, public")

        # schema_migrations: one row per migration, correct sha256 on the initial.
        rows = conn.execute(
        ...
        assert rows[1][1] == "0002_failure_mode_seed_key.sql"
        assert rows[2][1] == "0003_model_artifact.sql"
        assert rows[3][1] == "0005_metrics_daily_ext.sql"  # 0004 reserved by Phase-4
...
def test_idempotent_restart(fresh_db_dsn: str) -> None:
    ...
    with psycopg.connect(fresh_db_dsn) as conn:
        ...
        assert applied_at_after == applied_at_before  # untouched
